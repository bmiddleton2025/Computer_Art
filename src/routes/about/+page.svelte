<script>
	import { Carousel, Controls } from 'flowbite-svelte';
	import { base } from '$app/paths';

	const images = $state([
		{
			src: `${base}/images/MorseCodeImage.png`,
			alt: ''
		},
		{
			src: `${base}/images/MorseCodeImage-1.png`,
			alt: ''
		},
		{
			src: `${base}/images/MorseCodeImage-2.png`,
			alt: ''
		}
	]);

	let index = $state(0);
</script>

<div class="mb-10 space-y-10">
	<div class="space-y-8">
		<p class="text-3xl font-bold sm:text-4xl md:text-5xl">Creators</p>
		<div class="flex flex-col items-center justify-around gap-6 space-y-6 sm:gap-8 md:space-y-0 lg:grid lg:grid-cols-2 lg:flex-row">
			<div class="w-full text-center sm:w-2/3 lg:w-1/2">
				<img
					alt="Bobbi Middleton"
					src="{base}/bobbi-middleton.jpg"
					class="mx-auto w-40 rounded-full border-2 border-black object-contain sm:w-48"
				/>
				<div class="text-xs sm:text-sm lg:text-lg">
					<h2 class="font-bold">Project Concept:</h2>
					<h2 class="font-semibold">Bobbi Middleton</h2>
				</div>
			</div>
			<div class="w-full text-center sm:w-2/3 lg:w-1/2">
				<img
					alt="Dakota Gullicksen"
					src="{base}/dakota2025.jpg"
					class="mx-auto w-40 rounded-full border-2 border-black object-contain sm:w-48"
				/>
				<div class="text-xs sm:text-sm lg:text-lg">
					<h2 class="font-bold">Software Developer:</h2>
					<h2 class="font-semibold">Dakota Gullicksen</h2>
				</div>
			</div>
		</div>
	</div>
	<div class="space-y-10">
		<p class="text-3xl font-bold sm:text-4xl md:text-5xl">Purpose</p>
		<p class="text-base font-semibold italic sm:text-lg lg:text-2xl">
			The purpose of this site is to bridge a gap in STEM around morse code. Morse code is largely an unplugged activity; this site changes
			text to Morse code then into an image. The morse code then becomes an art piece visually and auditorily. I plan to have students write
			poems and have hidden meaning in their art.
		</p>
	</div>
	<div class="space-y-10">
		<p class="text-3xl font-bold sm:text-4xl md:text-5xl">Examples</p>
		<div class="mx-auto items-center">
			<Carousel {images} classes={{ slide: 'w-fit h-4/5' }} class="flex justify-center" bind:index duration={10000}>
				<Controls />
			</Carousel>
		</div>
	</div>
</div>
